...
If we take its derivative with respect to $x$, we would have:

$$
\frac{\partial f} {\partial x} = 2x + \cos(x + y^2)
$$

The symbol $\partial$ is called the partial derivative because $f$ take in two inputs, $x$ and $y$. And we partially taking the derivative with respect to one of them.
...
We will discuss the rules later. What important is that the symbolic method give us new functions each with respect to different inputs.
