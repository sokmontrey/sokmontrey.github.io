<script>
	...
	onMount(() => {
		...
		const loop = () => {
			ctx.fillStyle = bg_color + "aa";
			ctx.fillRect(0, 0, w, h);
			for (let i = 1; i < 10; i++) {
				const hex_opacity = Math.floor((9 - i) * 5)
					.toString(16)
					.padStart(2, "0");
				drawCircle(mouse_pos[0], mouse_pos[1], i / 2, nt_color + hex_opacity);
			}
		...
		const ripple = (e, x, y) => {
			...
			y = y || mouse_pos[1];
			for (let i = 0; i < 15; i++) {
				const hex_opacity = Math.floor((25 - i) * 5)
					.toString(16)
					.padStart(2, "0");
				setTimeout(() => {
					drawCircle(x, y, i, nt_color + hex_opacity, letters);
				}, i * 70);
			}
		};
