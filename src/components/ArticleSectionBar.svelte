<script>
	...
	onMount(() => {
		sections_el = Array.from(document.querySelectorAll("h1"))
			.map((el, i) => {
				el.id += `section-${i}`;
				return el;
			});
...
<aside
	class="hidden w-[100px] lg:flex flex-col justify-end fixed bottom-8 left-8 h-[100vh] font-mono"
>
	<ul class="">
		{#each sections as section, i}
			...
			>
				<button
					class="cursor-pointer text-left py-1 font-mono"
					on:click={() =>
